...
struct Comb {
    int64_t n;
    vector<mint> fac, invfac, pow2;
    Comb(): n(0) { }
    Comb(int64_t _n): n(0LL) { init(_n); }
    void init(int m) {
        if (m <= n) return;
        fac.assign(m + 1, 0LL);
        invfac.assign(m + 1, 0LL);
        pow2.assign(m + 1, 0LL);
        pow2[0] = mint(1LL);
        for (int i = 1; i <= m; i++) {
            pow2[i] = pow2[i - 1] * mint(2LL);
        }
        // factorials
        fac[0] = mint(1);
        for (int i = 1; i <= m; i++) {
    ...
    // pow(2, m)
    mint P2(int m) {
        if (m > n) return pow_mod(2, m, mod);
        return pow2[m];
    }
    // A(n, m) = n! / (n - m)!
    mint A(int n_, int m_) {
        if (m_ < 0 || m_ > n_) return mint(0);
        if (n_ > n) init(2 * n_);
        return (fac[n_] * invfac[n_ - m_]);
    }
    // C(n, m) = n! / (m! * (n - m)!)
    mint C(int n_, int m_) {
        if (m_ < 0 || m_ > n_) return mint(0);
        if (n_ > n) init(2 * n_);
        return (fac[n_] * invfac[m_] * invfac[n_ - m_]);
    }
};
Comb comb(1e6);
```
...
```C++
// 关于连乘的除法 % 0 操作的安全处理方式
// 跟具体有几个 mod 没关系，只是一种数字上的处理，没有什么实际含义，不支持 + -
struct safe_pro {
    int64_t zero = 0LL;
    mint prod = 1LL;
    ...
    safe_pro(int64_t x) : safe_pro(mint(x)) {}    

    void mul(const mint& x) {
        if (x.val() == 0) zero++; 
        else prod *= x;
    }
    void div(const mint& x) {
        if (x.val() == 0) zero--; 
        else prod /= x;
    }
    friend safe_pro operator*(safe_pro& l, const safe_pro& r) { 
        if (r.val() == 0) l.zero++; 
        else l.prod *= r.prod;
        return l;
    }
    safe_pro operator*=(const safe_pro& r) {
        return (*this) = (*this) * r; 
    }
    friend safe_pro operator/(safe_pro& l, const safe_pro& r) { 
        if (r.val() == 0) l.zero--; 
